...
class Settings:
    ...
    github_repo: str = field(default_factory=lambda: os.getenv("GITHUB_REPO", "xamidovasadbekdev-arch/Personal_blog"))
    ...
    contact_from: str = field(default_factory=lambda: os.getenv("CONTACT_FROM", "Portfolio <onboarding@resend.dev>"))
    # Sites allowed to call this API from the browser.
    allowed_origins: list[str] = field(
        default_factory=lambda: _list(
            os.getenv(
                "ALLOWED_ORIGINS",
                "https://xamidovasadbek.dev,https://www.xamidovasadbek.dev,http://localhost:5173",
            )
        )
    )
